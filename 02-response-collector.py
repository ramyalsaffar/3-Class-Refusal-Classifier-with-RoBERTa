...
class ResponseCollector:
    ...
    def __init__(self, claude_key: str, openai_key: str, google_key: str, config: Dict):
        """
        ...
        self.config = config
        self.models = config['data']['models']
    ...
    def collect_all_responses(self, prompts: Dict[str, List[str]]) -> pd.DataFrame:
        ...
        Returns:
            DataFrame with columns: [prompt, response, model, expected_label, category, timestamp]
        """
        all_data = []
    ...
    def _query_model(self, model_name: str, prompt: str, max_retries: int = 3) -> str:
        """
...
if __name__ == "__main__":
    ...
    for prompt_type in ['hard_refusal', 'soft_refusal', 'no_refusal']:
        filepath = os.path.join(data_raw_dir, f"prompts_{prompt_type}.json")
    ...
    collector.save_responses(responses_df, config['paths']['data_responses'])
