...
class ConfidenceAnalyzer:
    ...
    def __init__(self, model, tokenizer, device, config):
        ...
        self.device = device
        self.config = config
        self.class_names = ['No Refusal', 'Hard Refusal', 'Soft Refusal']
    ...
    def plot_confidence_distributions(self, labels: List, confidences: List, output_path: str):
        ...
        for class_idx in range(3):
            ...
            axes[class_idx].hist(class_confidences, bins=20, alpha=0.7, edgecolor='black')
            axes[class_idx].set_title(f'{self.class_names[class_idx]}')
            axes[class_idx].set_xlabel('Confidence')
        ...
        plt.close()
        print(f"\n✓ Saved confidence distributions to {output_path}")
