...
# 2. Superior performance on safety/toxicity detection tasks
# 3. Effective at capturing nuanced linguistic patterns
# 4. Bidirectional attention ideal for "soft refusal" detection
# 5. Production-ready with Hugging Face ecosystem
#
# IMPROVEMENTS:
...
class RefusalClassifier(nn.Module):
    """Enhanced RoBERTa-based classifier for refusal detection."""
    ...
    def __init__(self, 
        ...
        # Note: transformers is already imported in 01-Imports.py
        ...
        # Enhanced classification head with layer norm
        ...
        # Model statistics
        self.total_params = count_parameters(self, trainable_only=False)
        self.trainable_params = count_parameters(self, trainable_only=True)
    ...
    def _print_model_info(self):
        ...
        print_banner("REFUSAL CLASSIFIER MODEL", char="=")
    ...
    def forward(self, 
        ...
        Returns:
            If no extra outputs: Logits (batch_size, num_classes)
    ...
    def predict_with_confidence(self, 
        ...
        if use_mc_dropout:
            ...
            all_probs = torch.cat(all_probs, dim=0)  # (n_samples, batch, classes)
    ...
    def freeze_roberta_layers(self, num_layers_to_freeze: int = None):
        ...
        if EXPERIMENT_CONFIG.get('verbose', True):
            print_banner(f"FREEZING LAYERS", char="─")
